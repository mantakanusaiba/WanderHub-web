...
const HolidayPage = ({ navigateTo }) => {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await fetch('https://wander-hub-webapi.vercel.app/api/holiday-packages/get-all-holiday-packages', {
          headers: {
  ...
  const applyFilters = async () => {
    try {
      ...
      const response = await fetch(`https://wander-hub-webapi.vercel.app/api/holiday-packages/get-holiday-packages?${queryParams.toString()}`, {
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },
