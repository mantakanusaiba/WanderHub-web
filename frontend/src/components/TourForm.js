...
const TourForm = ({ packageId, onBookingComplete, navigateTo }) => {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      const response = await fetch('https://wander-hub-webapi.vercel.app/api/tourBook', {
        method: 'GET',
        headers: {
  ...
  const handleSaveEdit = async (e) => {
    ...
    const url = editingBookingId
      ? `https://wander-hub-webapi.vercel.app/api/tourBook/${editingBookingId}`
      : `https://wander-hub-webapi.vercel.app/api/tourBook/${packageId}`;
  ...
  const handleDelete = async (bookingId) => {
    ...
    try {
      const response = await fetch(`https://wander-hub-webapi.vercel.app/api/tourBook/${bookingId}`, {
        method: 'DELETE',
      });
